...
def load_system_rows(file_bytes: bytes) -> List[SystemRow]:
    ...
    for _, r in df.iterrows():
        ...
        rows.append(SystemRow(
            style=str(r['Style']).strip() if pd.notna(r.get('Style')) else '',
            nrf_color=str(r['NRF Color']).strip() if pd.notna(r.get('NRF Color')) else '',
            account=str(r['Account']).strip() if pd.notna(r.get('Account')) else '',
            color=str(r['Color']).strip() if pd.notna(r.get('Color')) else '',
            pack_ratio=str(r['Pack Ratio']).strip() if pd.notna(r.get('Pack Ratio')) else '',
            ...
            reason=str(r['Reason']).strip() if pd.notna(r.get('Reason')) else '',
